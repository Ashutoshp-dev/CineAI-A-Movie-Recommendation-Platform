...
const LikedMovies = ({
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      if (!likedMovies || likedMovies.length === 0) {
        console.warn("⚠️ No liked movies available for recommendation.");
        return;
      }

      try {
        const res = await fetch(`${baseURL}/recommend/liked`, {
          method: "POST",
          headers: {
  ...
  return (
    <div className="px-5 py-20 bg-gray-950 text-white min-h-screen">
      <h1 className="text-3xl mb-5">Liked Movies</h1>
      {likedMovies.length === 0 ? (
        ...
        <>
          ...
          {recommendations.length > 0 && (
            <>
              <h2 className="text-3xl mt-10 mb-5">You may also Like</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 bg-gray-950 h-full w-full gap-5">
